...
/*
AnsiString __fastcall GenerateUniqueName()
{
    TGUID   guid;

    CreateGUID( guid );

    std::string     sstr( GUIDToString( guid ).c_str() );

    sstr.erase( std::remove( sstr.begin(), sstr.end(), '{' ) );
    sstr.erase( std::remove( sstr.begin(), sstr.end(), '}' ) );
    sstr.erase( std::remove( sstr.begin(), sstr.end(), '-' ) );
    return AnsiString( sstr.c_str() );
}
*/
...
{
private:
    std::auto_ptr<TDatabase>    mDatabase;
    static  int                 mNameGenerator;
...
public:
    ...
    ~cDataConnection();
    TDatabase *GetDatabase()                { return ( mDatabase.get() ); }
};
...
//******    cDataConnection
//***********************************************************************
int     cDataConnection::mNameGenerator = 0;

cDataConnection::cDataConnection( const char *connection_string )
    : mDatabase(new TDatabase( 0 ))
{
    std::auto_ptr<TStringList>      slist( new TStringList );

    slist->Text = connection_string;
    mDatabase->AliasName = slist->Strings[0];
    slist->Delete( 0 );
    mDatabase->Params = slist.get();
    mDatabase->DatabaseName = GenerateUniqueName();
...
AnsiString __fastcall cDataConnection::GenerateUniqueName()
{
    return IntToStr( mNameGenerator++ );
}
...
{
    mQuery.reset( new TQuery(0) );
    mQuery->DatabaseName = mDataConnection->GetDatabase()->DatabaseName;
    mQuery->SQL->Text = sql;
    mQuery->Open();
}
...
{
    typedef std::vector<FieldFieldPair>::iterator   FieldFieldPair_it;

    bool        result = false;
    Variant     v;

    for ( FieldFieldPair_it it = mFieldPairMap.begin(), eend = mFieldPairMap.end() ; it != eend ; ++it )
    {
        v = it->mNativeField->Value;

        if ( VarIsNull( v ) )
            values_acceptor->FieldValue( it->mFieldName.c_str(), it->mFieldDataType, it->mData, 0, it->mFieldDataSize );
        else switch ( it->mFieldDataType )
        {
            case cFieldDataType_ftBool :
                {
                    bool    a = v;

                    result = values_acceptor->FieldValue( it->mFieldName.c_str(), it->mFieldDataType, it->mData, &a, it->mFieldDataSize );
                }
                break;
            ...
            case cFieldDataType_ftString :
                {
                    ...
                    result = values_acceptor->FieldValue( it->mFieldName.c_str(), it->mFieldDataType, it->mData, a.c_str(), it->mFieldDataSize );
                }
                break;
