...
template <typename RefCounter> class shared_in_base
{
    template <typename U> friend class shared_in_ptr;
    RefCounter  shared_in_count;
public:
    shared_in_base() : shared_in_count(0)           {} // empty
};
...
public:
    ...
    explicit shared_in_ptr( T* p ) : px(p)
    {
        if ( px )
            ++px->shared_in_count;
    }

    template <class Y> explicit shared_in_ptr( Y * p ) : px( p )
    {
        if ( px )
            ++px->shared_in_count;
    }

    template <class Y> shared_in_ptr( shared_in_ptr<Y> const & r ) : px(r.px)
    {
        if ( px )
            ++px->shared_in_count;
    }
    ...
    bool unique() const                             { return px && px->shared_in_count == 1; }
    void swap(shared_in_ptr<T>& other)              { std::swap( px, other.px ); }
private:
...
template <typename T, typename U> inline bool operator==(const shared_in_ptr<T>& a, const shared_in_ptr<U>& b)
{
    return a.get() == b.get();
}
